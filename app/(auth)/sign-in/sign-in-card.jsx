...
const SignInContent = ({ initialReturningVisitor }) => {
  const [isLoadingProvider, setIsLoadingProvider] = React.useState(null);
  const [isReturningVisitor, setIsReturningVisitor] = React.useState(
    initialReturningVisitor,
  );
  const searchParams = useSearchParams();
  const oauthError = searchParams.get("error");
  const oauthErrorText = describeOAuthError(oauthError);

  // Mark the visitor as "seen" on their first visit so the next time they land
  // on the sign-in page it greets them as a returning user.
  React.useEffect(() => {
    if (!isReturningVisitor) {
      document.cookie =
        "aither_visited=1; path=/; max-age=31536000; samesite=Lax";
...
const SignInCard = ({ initialReturningVisitor }) => {
  return (
    <Suspense fallback={null}>
      <SignInContent initialReturningVisitor={initialReturningVisitor} />
    </Suspense>
  );
};
