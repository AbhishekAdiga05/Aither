...
// fail in practice (timeout, socket hang up, agentic-only 403, persistent
// 429, paid-only 402, etc.). Shared by both the model picker and the chat
// allow-list so a broken model can never be selected or invoked.
export const KNOWN_BROKEN_MODEL_IDS = new Set([
  "nex-agi/nex-n2.5-mini:free",          // TIMEOUT / socket hang up (was the old DEFAULT!)
  "nvidia/nemotron-3-ultra-550b-a55b:free", // TIMEOUT / socket hang up
  "thinkingmachines/inkling-small:free", // 403 agentic-only
  "thinkingmachines/inkling:free",       // 403 agentic-only
...
/** Filter a model list (from the live catalog) to remove known-broken ones. */
export function filterKnownBroken(models) {
  return Array.isArray(models)
    ? models.filter((model) => !KNOWN_BROKEN_MODEL_IDS.has(model.id))
    : models;
}

// Fail-safe: returns live free model ids, or the static snapshot on failure.
// Known-broken models are always stripped so the /api/chat allow-list can
// never approve a model we already know hangs or errors.
export async function getAllowedFreeModelIds() {
  try {
    const models = filterKnownBroken(await getFreeModels());
